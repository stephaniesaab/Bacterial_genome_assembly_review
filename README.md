...
A sample representation of the aligned reads in contig 2 with the reference genome are shown in IGV visualization in *Figure 1*, highlighting the adequate coverage of the assembly against the reference genome. IGV highlights the efficacy and accuracy of the assembly as it shows the raw reads aligned in the ass
 which can then be validated by mapping raw sequencing reads (BAM/CRAM files) back to these novel contigsOverall, this project produced a successful step-by-step script to assemble an S. enterica genome de novo from ONT long-reads. 
